	...
	void toDo(ActionEvent event) throws JSONException, IOException {
        ...
        String response;
      		...
      		try {
      	          ...
      	          URL url = new URL("http://"+myIp+":"+myPort+"/myProject/ActivateOrder");
		...
		if(event.getSource().equals(activate)){
			if(status==true){
			...
			if(status==false){
	//		msg.setVisible(true);
			loader = new FXMLLoader(getClass().getResource("/ActivateOrder.fxml" ));
			ActivateOrderController controller = new ActivateOrderController();
			loader.setController(controller);
			}
