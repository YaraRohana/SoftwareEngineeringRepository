...
public class FollowOrdersAndSubscriptions {
	@FXML
	...
	void toDo(ActionEvent event) throws JSONException, IOException {
        ...
        String response;
      		...
      		try {
      	          ...
      	          URL url = new URL("http://"+myIp+":"+myPort+"/myProject/FollowOrderAndSubscriptions");
		...
		if(event.getSource().equals(follow)){
			if(status==true){
			...
			if(status==false){
	//		msg.setVisible(true);
			loader = new FXMLLoader(getClass().getResource("/FollowOrdersAndSubscriptions.fxml" ));
